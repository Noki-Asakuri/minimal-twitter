...
const BUNDLE_FILE = "./scripts/bundle-extension.js";
...
function updateVersion(newVersion) {
  const content = fs.readFileSync(BUNDLE_FILE, "utf8");
  const updatedContent = content.replace(/version:\s*"\d+\.\d+\.\d+"/, `version: "${newVersion}"`);
...
function updateXcodeProject(newVersion, newBuildNumber) {
  ...
  content = content.replace(/MARKETING_VERSION = [\d.]+;/g, `MARKETING_VERSION = ${newVersion};`);
...
async function main() {
  try {
    ...
    console.log(
      `Bumping ${bumpType} version: ${currentVersion} → ${newVersion} (build ${currentBuildNumber} → ${newBuildNumber})`,
    );
