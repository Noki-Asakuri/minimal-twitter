...
import addTooltip from "@content/modules/utilities/add-tooltip";
...
export function addTypefullyComposerPlug(): void {
...
export function saveCurrentReplyToLink(): void {
  const reply = Array.from(document.querySelectorAll('[data-testid="reply"]'));
  if (!reply.length) return;
  function logLink(event: Event): void {
    if (!(event.target instanceof Element)) return;
    const tweet = event.target.closest('[data-testid="tweet"]');
    const tweetLinks = tweet?.querySelectorAll<HTMLAnchorElement>("a[role='link']");
...
export function addTypefullyReplyPlug(): void {
  setTimeout(() => {
    const modal = document.querySelector('[aria-labelledby="modal-header"][role="dialog"]');
...
export function addTypefullySecurityAndAccountAccessPlug(): void {
...
export function addTypefullySchedulePlug(): void {
  const scheduleConfirmButton = document.querySelector<HTMLElement>(
    '[data-testid="scheduledConfirmationPrimaryAction"]',
  );
  if (scheduleConfirmButton && !document.getElementById("typefully-schedule-button")) {
    const scheduleButton = scheduleConfirmButton.cloneNode(true) as HTMLElement;
...
/* ----------------------------- Typefully Utils ---------------------------- */
export function createTypefullyLinkElement(id: string, className: string): HTMLAnchorElement {
...
export function createTypefullyLogo(): HTMLDivElement {
...
export function getCurrentTextAndSendToTypefully(
  replyingToLink?: string | null,
  utm_content?: string,
): void {
  ...
  while (true) {
    if (document.querySelector(`[data-testid="tweetTextarea_${tweetTextAreaNumber}"]`)) {
      ...
      tweetTextItems.forEach((item, index) => {
        ...
        function isTagOrMention(item: HTMLElement): boolean {
          return Boolean(item.parentElement?.parentElement?.attributes.getNamedItem("style"));
        }
